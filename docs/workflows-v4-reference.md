...
A send action's `fields.content_id` **is** the marketing email id, so join it
...
- `startActionId`: the entry point of the graph.
...
> HubSpot's full catalog is larger and can grow, so treat an unknown id as
...
- `IN_LIST` + `NOT_IN_LIST` on the **same** list is a full partition, safe with
  no default.
- Two **positive** conditions (e.g. "in list A" / "in list B") do **not**
  partition: a contact in neither falls through, so it needs a default.
...
1. **Pull** the flow JSON (and every list it references). Audit the JSON, not the
   ...
   to the original (diff the JSON; only metadata like `id` / `name` / `uuid`
   should differ).
...
5. **Walk the flow** and check the recurring failure patterns: branch fallback
   ...
   and suppression/marketing guards.
